...
class WebUIHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            # Attempt to resolve from package resources
            resource_path = files("neuralclaw.webui").joinpath(path)
            
            # If not a file (directory navigation, etc) or raises exception serving index.html
            if not resource_path.is_file():
                resource_path = files("neuralclaw.webui").joinpath("index.html")
                
            content = resource_path.read_bytes()
            # Guess type from file extension
            content_type, _ = mimetypes.guess_type(path)
            # Default to index.html type if we fell back
            if not content_type or not path.endswith(resource_path.name):
                content_type, _ = mimetypes.guess_type(resource_path.name)
